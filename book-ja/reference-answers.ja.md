...
> 仮説を階層化し、段階的に検証します。低コストの表層的な仮説（H1/H2 のプロンプト類）を並行して先に走らせ、高価な深層的な仮説（H5/H6 のモデル変更）は条件付きで起動します。統計的には：まず標準誤差を保守的なふるいとして使い、届かない点差を除外します。同一バッチのタスクにはペア分析（McNemar）を用い、独立比較よりはるかに鋭敏です。点差がノイズの帯域幅より小さければ意思決定しません。評価集が期待される利得を判別できないときは、評価集の拡張を試みます。
...
> ルーティングのシグナル：タスク記述にカウントや複雑な推論の特徴が含まれる（H7 は 1〜2 秒の高速 LLM で事前判定）。能力ラベル行列の中で過去の失敗が集中するタスクの種類。実行中にバックトラックの回数が増える、ツールが連続してエラーを返す、といった不確実性のシグナル。有害な場面：遅延に敏感なリアルタイム対話（音声、UI 操作）。単純なタスクではコストが 3〜5 倍になるのは純粋な無駄です。
